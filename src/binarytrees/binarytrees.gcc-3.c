...
int main(int argc, char *argv[])
{
	...
	const intnative_t minimum_Tree_Depth = 4,
					  maximum_Tree_Depth = atoi(argv[1]) < minimum_Tree_Depth + 2 ? minimum_Tree_Depth + 2 : atoi(argv[1]);
...
#pragma omp for nowait schedule(static, 1) collapse(2) private(iterations)
			 ...
			 tree_Depth <= maximum_Tree_Depth; tree_Depth += 2)
		{
			...
			// #pragma omp parallel reduction(+:total_Trees_Checksum)
			for (iterations =
				 ...
				 iterations > 0; --iterations)
			{
				...
				tree_Checksums[(tree_Depth - minimum_Tree_Depth) / 2] += compute_Tree_Checksum(
